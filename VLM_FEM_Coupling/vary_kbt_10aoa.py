...
if __name__ == '__main__':
    ...
    bss_u = np.append(b_u,
                      b_u)  # twice the dimensions in SS form since we also consider nodal velocities in the state vector
    ...
    aoa0 = 10 / 180 * pi # choose 10 deg as aoa since it should lead to more noticeable effects when changing the KBT

    val = 0.1 * (data['fem']['EI'] * data['fem']['GJ'])**0.5

    for KBT in [ 0, + val, - val]: # loop through KBT values

        data['fem']['KBT'] = KBT # update the value in the dict
        fem = fe.initialise_fem(data['fem']) # and in the fem object
        ...
        for j in range(1, 4):
            ax[j].plot(y_nd, xi[j - 1::3], '--', label=f'{KBT = :.5f}')
            # ax[j].plot(y_nd, xi_rigid[j - 1::3], '-.')
            ax[j].set_ylabel(lbl_y[j])
    ax[3].legend(loc='best')
    ax[-1].set_xlabel('span, [m]')
    fig.tight_layout()
    # fig.savefig('./Plots/Vary_KBT_high_speed_test.png', format='png', dpi=300)
    fig.savefig('./Plots/Vary_KBT.png', format='png', dpi=300)