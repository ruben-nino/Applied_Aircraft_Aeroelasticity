...
if __name__ == '__main__':
    ...
    for aoa0_deg in [ 1 , 3 , 10]: # loop through AoAs

        aoa0 = aoa0_deg / 180 * pi
        ...
        for j in range(1, 4):
            ax[j].plot(y_nd, xi[j - 1::3], '--', label=r'$\alpha_0$ = '+f'{aoa0_deg:.0f} deg')
            # ax[j].plot(y_nd, xi_rigid[j - 1::3], '-.')
            ax[j].set_ylabel(lbl_y[j])
    ax[2].legend(loc='best')
    ax[-1].set_xlabel('span, [m]')
    fig.tight_layout()
    fig.savefig('./Plots/Vary_AoA_.png', format='png', dpi=300)