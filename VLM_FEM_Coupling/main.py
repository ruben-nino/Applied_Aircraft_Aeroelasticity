...
if __name__ == '__main__':
    ...
    bss_u = np.append(b_u,
                      b_u)  # twice the dimensions in SS form since we also consider nodal velocities in the state vector # actually, this is useless since we don't use SS form
    ...
    # aoa0 = 10 / 180 * pi # choose 10 deg as aoa since it should lead to more noticeable difference from rigid to flex
    aoa0 = 3 / 180 * pi # assignment prescribes 3 deg actually
    ...
    for i in range(10):  # update aerodynamic forces based on structural displacements # 10 for high speed test
        # AoA correction:
        ...
        for j in range(n_v):  # loop through the columns of panels (spanwise)
            ...
            closest_nd_idx = (
                np.abs(y_nd - y_control[0, j])).argmin()  # 0th element has the same y as any other in the column
    ...
    lbl_y = ['Lift [N]', r'$\theta$, [rad]', 'v, [m]',
             r'$\beta$, [rad]']  # NOTE beta here is the y derivative of v ( =rotation in EB beams)

    ax[0].plot(y_nd[:], f[:], 'o')
    ax[0].plot(y_nd[:], f_rigid[:], '*')
    ax[0].set_ylabel(lbl_y[0])
    for j in range(1, 4):
        ax[j].plot(y_nd, xi[j - 1::3], '--', label=f'Flexible')
        ax[j].plot(y_nd, xi_rigid[j - 1::3], '-.', label=f'Rigid')
        ax[j].set_ylabel(lbl_y[j])
    ax[3].legend(loc='best')
    ax[-1].set_xlabel('span, [m]')
    fig.tight_layout()
    # fig.savefig('./Plots/flex_vs_rigid.png', format='png', dpi=300)\
    print(f'Aero Load , {f.max() - f_rigid.max():.3e}')
    print(f'Twist  ,{xi[0::3].max() - xi_rigid[0::3].max():.3e} ')
    print(f"Bending , {xi[1::3].max() - xi_rigid[1::3].max():.3e} ")
    fig.savefig('./Plots/flex_vs_rigid_high_speed_test.png', format='png', dpi=300)
